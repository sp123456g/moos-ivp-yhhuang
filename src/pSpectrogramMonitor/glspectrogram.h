...
// 7/16/18: integrate into MOOSDB, yhhuang
...
struct Param {           // parameters object
  ...
  int twowinsize;        // power of 2 giving DFT win_size (N) in samples,(window長2的幾次方)
};                       // note trailing ;
Param param;           // global parameters object

const char *notenames[] = { "C","C#","D","Eb","E","F","F#","G","G#","A","Bb","B"};  // 1/17/16

// using gettimeofday to access the time in usec and second, include in sys/time.
...
//used to set up a text in bigger font
void drawText(float x, float y, char *string) {
  ...
  for (i = 0; i < len; i++)
  { 
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, string[i]); //font size
  }
}

//used to set up a text in smalller font
void smallText(float x, float y, char *string) {
  ...
  for (i = 0; i < len; i++)
  {
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, string[i]); //font size
  }
}
...
//    Hz_per_pixel = 1.0F / (win_size*dt);
    ...
    // pthread_create detail:
    // first  parameter is the pointer pointer to the index of thread(線程識別元)
...
class scene
{
  ...
  void slice() // simple graph of spectral slice with log power scale
  {
    glMatrixMode(GL_MODELVIEW);
    glPushMatrix(); // use modelview matrix to xform [-tshow,0]x[-1,1] somewhere
    glTranslatef(0.15,0.05,0);   // decide the location of 0,0 of the graph 
    float max_Hz = ai->Hz_per_pixel*ai->n_f;
//    std::cout<<"max_Hz="<<max_Hz<<std::endl;
    glScalef(0.3/max_Hz,0.00075,1.0);  // x-scale for freq-units, y-scale for dB axis scale factor
    glLineWidth(1);
    ...
    glBegin(GL_LINE_STRIP);
    float sensitivity = -165;
    for( int i=0; i<ai->n_f; i++ )
      glVertex2f(i*ai->Hz_per_pixel, 20*log10(ai->specslice[i])-sensitivity);
    glEnd();
    char xlab[] = "f(Hz)", ylab[] = "I(dB)";   // axis labels
    drawAxes(0-1e-7, max_Hz, 0, 220, 1.0, 1.0, 1.0, 0, xlab, ylab);
    glPopMatrix();
  }
  ...
  {
    float x0=0.05, y0=0.25;    // bot-left location in viewport (as unit square)
